...
CLI.prototype.showHelp = function(){
  ...
  var markdownOptions = {
    // Colors
    code: this.color.yellow,
    blockquote: this.color.gray.italic,
    html: this.color.gray,
    heading: this.color.reset,
    firstHeading: this.color.white.underline.bold,
    hr: this.color.reset,
    listitem: this.color.reset,
    table: this.color.reset,
    paragraph: this.color.reset,
    strong: this.color.bold,
    em: this.color.italic,
    codespan: this.color.yellow,
    del: this.color.dim.gray.strikethrough,
    link: this.color.blue,
    href: this.color.blue.underline, 
    ...
    tableOptions: {chars: { "top": "" , "top-mid": "" , "top-left": "" , "top-right": ""
  ...
  marked.setOptions({
    renderer: new TerminalRenderer(markdownOptions)
  }); 

  this.stdout(marked(this.help));
  this.done = true;
};
  ...
  if (!this.done){
    ...
    if (this.init.runcom !== undefined){
      ...
      if (path.isAbsolute(this.init.runcom)){ 
        rc = JSON.parse(fs.readFileSync(this.init.runcom).toString());
      } else {
